...
static void tryConnectBleAddonsFromStart()
{
  ...
  if (addonsIsCoyoteEnabled()) {
    CoyoteSetAddonEnabled(true);
    if (CoyoteTryConnectNow()) {
      LogDebug("Coyote addon connected over BLE from Start flow");
    }
  } else {
    CoyoteSetAddonEnabled(false);
  }
}
